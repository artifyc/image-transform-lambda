...
class ImageResize():
    ...
    def __init__(self, config_file='image_resize_settings.json'):
...
def lambda_handler(event, context):
    ...
    image_resize.verify_buckets()

    for object in event.get('Records'):
        object_key = unquote_plus(object['s3']['object']['key'])
        object_extension = image_resize.check_extension(object_key)

        # Source file
        obj = image_resize.s3.Object(
            bucket_name=image_resize.source_bucket,
            key=object_key,
        )
        obj_body = obj.get()['Body'].read()

        # Resized files
        for bucket in image_resize.destination_buckets:
            bucket_name = bucket.get('name')
            bucket_size = bucket.get('width_size')

            resized_image = image_resize.resize_image(
                obj_body,
                object_extension,
                bucket_size
            )
